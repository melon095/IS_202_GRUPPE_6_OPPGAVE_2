...
public class HindranceServiceTests
{
    ...
    public async Task AddPoints_ShouldAddHindrancePoints()
    {
        ...
        // Assert
        var addedPoints = await _dbContext.HindrancePoints.Where(p => p.HindranceObjectId == hindranceObjectId).ToListAsync();
    ...
    public async Task GetAllObjectsSince_ShouldReturnObjects()
    {
        ...
        var report = new ReportTable { Id = Guid.NewGuid(), ReportedById = userId, ReportedBy = user, Title = "", Description = "", ReviewStatus = ReviewStatus.Draft};
        ...
        var hindranceType = new HindranceTypeTable { Id = Guid.NewGuid(), Name = "Test Type", GeometryType = GeometryType.Point };
